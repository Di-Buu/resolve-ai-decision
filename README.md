...
当前实现是受约束的模块化 AI 工作流。它的 Agentic 特征体现在：围绕完成决策这一目标，根据已保存的状态、证据和异议分阶段分析，并在条件不满足时暂停、回退或恢复；但模型不会自主规划任务或调用外部工具，也不是多个自治 Agent 自由沟通和协商的正式 Multi-Agent 系统。

## 当前 MVP 已实现能力

- 用自然语言描述产品决策，由 AI 整理目标、相关人员、限制和信息缺口。
- 在本地切换参与者身份；AI 将每个人的原话整理成目标、底线、可商量部分、依据和假设，再由本人修改并确认。
- AI 把确认后的观点汇总成共同目标、各方重点、真正分歧和证据缺口，由负责人确认大家讨论的是同一个问题。
- 由负责人完成人工需求判断；需要更多依据时进入补充信息，暂缓或结束时保存原因，并允许以后重新开始讨论。
- 生成 2—3 个机制和取舍方向不同的候选方案，展示新思路、行动步骤、满足与牺牲、所需证据、回退办法和成功信号，再分别检查限制、证据和风险。
- 方案缺少证据时，可以补充信息、重新生成并重新检查，而不是停在不可操作的页面。
- 各参与者可提交接受、顾虑或反对意见，AI 分析异议影响了哪些方案。
- 存在未处理异议或关键条件未满足时，阻止进入最终确认；被新证据判定失效的方案不会因为切换选择而重新变成可用。
- 将决定、理由、代价、风险、责任人和重新讨论条件保存为决策记录。
- 使用 SQLite 保存过程，刷新页面或重启服务后仍可继续。
- 浏览器地址会记录当前决策和页面，可以使用前进、后退或阶段导航回看已经完成的步骤。
- 未配置模型时，可以从头体验预置的“AI 自动退款”产品决策示例；示例文字和 AI 结果均已准备好，自定义任务失败时不会用示例数据冒充实时结果。
...
- **用户现象**：早期版本只让参与者确认一段原话，没有先展示 AI 如何理解目标、底线、依据和假设；候选方案也容易停留在延期、缩小范围或简单折中。
- **原因**：观点收集缺少“个人结构化确认 → 共同问题确认”两层理解校验，方案输出也没有要求解释新思路、执行步骤、回退办法和成功信号。
- **修改**：加入个人观点结构化编辑、负责人确认共同问题两个人工节点；方案生成增加分群、可逆试点、影子模式、分阶段上线、人机协同和低成本验证等探索方向，并明确禁止只改写各方原话。
- **结果**：后续分析只使用本人确认过的结构化观点；团队先对共同目标和真正分歧达成一致，再比较内容更完整、差异更明确的方案。
...
| 纵向 Flow 成本与耗时（新版） | 平均约 123 秒；三例通过运行合计估算模型成本约 ¥0.4041 | 2026-09-15 在加入个人观点与共同问题确认后的当前链路上运行 |

以上均为内部工程评测，用于验证工作流与模型选择，不代表真实用户效果或业务收益。Smoke 和 Holdout 的通过率是项目规则检查结果，不是通用准确率；3 个纵向案例是本次运行最终均通过，不代表所有输入都能首次通过。改造前 Flow 基线为平均约 47 秒、三例合计约 ¥0.1223；新版链路当前为平均约 123 秒、三例合计约 ¥0.4041，说明新增确认环节带来了额外调用成本与等待时间。
...
| `POST /api/decisions/sample-ai-refund/reset` | 将预置示例恢复到尚待产品负责人确认观点的初始状态 |
| `POST /api/decisions/:id/events` | 保存个人观点与共同问题确认、需求判断、方案选择、审阅、异议和最终决定 |
| `POST /api/decisions/:id/ai/viewpoint` | 将一位参与者的原话整理成可修改、可确认的结构化观点 |
| `POST /api/decisions/:id/ai/brief` | 汇总已确认观点，形成需要负责人确认的共同问题说明 |
...
- 进入诊断阶段后，已确认的个人观点目前只能回看，不能在原决策内直接修改；需要新建决策，预置示例则可以从头重置。
