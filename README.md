...
Resolve 是一个面向多场景复杂协作决策的 AI Agentic 系统。它服务于工作、生活和学习中一个或多个参与者共同面对的多目标、多约束、信息不完整和持续变化问题，把散落的目标、限制、分歧与证据整理成可确认、可检查、可追溯的决策过程，并把最终选择和责任留给人。

产品经理和项目负责人不是 Resolve 的全部目标用户，而是当前“工作 → 产品与研发”MVP 分支的主要组织者。当前本地可运行纵向 MVP V0.7 只开放这一分支；生活、学习及其他工作领域保留扩展入口。
...
这些分支复用同一套复杂决策能力：理解参与者与目标，区分硬约束、偏好、证据和假设，发现冲突与信息缺口，提出高价值问题，探索并检查方案，处理异议和变化，最后由人确认并留下决策记录。

这里的 Agentic 指多个受约束的 AI 任务在状态机中协作推进决策，不代表已经实现多个自治 Agent 自由协商的正式 Multi-Agent 系统。
